...
function validateLoginForm() {
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', function(e) {
            ...
            // 模拟登录验证
            handleLogin(account.value, password.value, 'account');
        });
    }
    ...
    if (tempForm) {
        tempForm.addEventListener('submit', function(e) {
            ...
            // 模拟临时码登录验证
            handleLogin(tempCode.value, '', 'temp');
        });
    }
}

// 处理登录逻辑
async function handleLogin(account, password, loginType) {
    const errorText = document.getElementById('error-text');
    ...
    try {
        ...
        errorText.textContent = '登录时发生错误，请重试';
        errorText.classList.remove('hidden');
    }
}
...
async function logout() {
    try {
        // 模拟API调用
        const response = await fetch('data/logout.json');
        const data = await response.json();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 在需要验证登录的页面检查登录状态
    const needsAuth = !window.location.pathname.includes('login.html');
    if (needsAuth) {
        checkLoginStatus();
        initSidebar();
        checkServerStatus();
