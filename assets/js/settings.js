function settingsApp() {
    return {
        ...
        webConfig: {
            host: '127.0.0.1',
            port: 8000,
            super_admin_account: 1377820366,
            disable_admin_login_web: false,
            enable_temp_login_password: true,
            ssl_enabled: false,
        ...
        mcdrPluginsUrl: 'https://api.mcdreforged.com/catalogue/everything_slim.json.xz',
        pimStatus: 'checking',
        ...
        async getConfig() {
            if (this.isDemoMode) {
                try {
                    // 演示模式：从本地数据文件获取配置
                    const response = await fetch('../data/guguwebui/config.json');
                    const config = await response.json();
                    
                    this.webConfig.host = config.host || '127.0.0.1';
                    this.webConfig.port = config.port || 8000;
                    this.webConfig.super_admin_account = config.super_admin_account || 1377820366;
                    this.webConfig.disable_admin_login_web = config.disable_other_admin || false;
                    this.webConfig.enable_temp_login_password = config.allow_temp_password || true;
                    this.aiModel = config.ai_model || 'Pro/deepseek-ai/DeepSeek-V3';
                    this.aiApiUrl = config.ai_api_url || 'https://api.siliconflow.cn/v1/chat/completions';
                    ...
                    this.webConfig.ssl_certfile = config.ssl_certfile || './config/guguwebui/ssl/cert.pem';
                    this.webConfig.ssl_keyfile = config.ssl_keyfile || './config/guguwebui/ssl/key.pem';
                    ...
                    console.log('演示模式：成功加载配置数据');
                } catch (error) {
                    console.error('演示模式：加载配置失败', error);
                    this.showNotification('演示模式：加载配置失败', 'error');
                }
                return;
            }
        ...
        async saveNetworkConfig() {
            if (this.isDemoMode) {
                this.showDemoModeNotification('保存网络设置');
                return;
            }
        ...
        async saveAccountConfig() {
            if (this.isDemoMode) {
                this.showDemoModeNotification('保存账户设置');
                return;
            }
        ...
        async toggleSetting(setting) {
            if (this.isDemoMode) {
                this.showDemoModeNotification('切换设置');
                return;
            }
        ...
        showDemoModeNotification(operation) {
            this.showNotification(`演示模式不支持${operation}操作`, 'info');
        },
        
        // 保存AI配置
        async saveAiConfig() {
            if (this.isDemoMode) {
                this.showDemoModeNotification('保存AI配置');
                return;
            }
        ...
        async validateApiKey() {
            if (this.isDemoMode) {
                this.showDemoModeNotification('验证API密钥');
                return;
            }
        ...
        async saveRepositories() {
            if (this.isDemoMode) {
                this.showDemoModeNotification('保存仓库配置');
                return;
            }
        ...
        async installPimPlugin() {
            if (this.isDemoMode) {
                this.showDemoModeNotification('安装PIM插件');
                return;
            }
        ...
        async saveHttpsConfig() {
            if (this.isDemoMode) {
                this.showDemoModeNotification('保存HTTPS设置');
                return;
            }
