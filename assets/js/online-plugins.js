    ...
    Alpine.data('onlinePluginsData', () => ({
        ...
        async loadPlugins(skipRepositoryLoad = false) {
            if (this.isDemoMode) {
                try {
                    ...
                    // 演示模式：直接从在线仓库获取数据
                    let repoUrl = 'https://api.mcdreforged.com/catalogue/everything_slim.json';
                    if (this.selectedRepository && this.selectedRepository.url) {
                        repoUrl = this.selectedRepository.url;
                    }
                    
                    console.log('演示模式：正在从仓库获取数据:', repoUrl);
                    const response = await fetch(repoUrl);
                    const data = await response.json();
                    
                    // 处理获取到的插件数据
                    if (data && data.plugins) {
                        // everything_slim.json 格式：plugins 是一个对象，需要转换为数组
                        const pluginsArray = [];
                        for (const [pluginId, pluginData] of Object.entries(data.plugins)) {
                            // 处理作者信息
                            let authors = [];
                            if (pluginData.meta?.authors && Array.isArray(pluginData.meta.authors)) {
                                authors = pluginData.meta.authors.map(authorName => {
                                    // 从全局authors对象中查找作者信息
                                    const authorInfo = data.authors?.authors?.[authorName];
                                    return {
                                        name: authorName,
                                        link: authorInfo?.link || null
                                    };
                                });
                            }
                            
                            // 获取最新版本信息
                            const latestVersion = pluginData.release?.latest_version || pluginData.meta?.version || '1.0.0';
                            const latestVersionIndex = pluginData.release?.latest_version_index || 0;
                            const latestRelease = pluginData.release?.releases?.[latestVersionIndex];
                            
                            // 获取下载次数 - 统计所有版本的下载次数总和
                            let downloads = 0;
                            if (pluginData.release?.releases && Array.isArray(pluginData.release.releases)) {
                                downloads = pluginData.release.releases.reduce((total, release) => {
                                    if (release.asset && release.asset.download_count) {
                                        return total + release.asset.download_count;
                                    }
                                    return total;
                                }, 0);
                            }
                            ...
                            if (latestRelease?.created_at) {
                                lastUpdateTime = latestRelease.created_at;
                            } else if (pluginData.release?.releases?.[0]?.created_at) {
                                lastUpdateTime = pluginData.release.releases[0].created_at;
                            }
                            ...
                            const plugin = {
                                ...
                                description: pluginData.meta?.description || {},
                                authors: authors,
                                latest_version: latestVersion,
                                last_update_time: lastUpdateTime,
                                downloads: downloads,
                                labels: pluginData.plugin?.labels || [],
                                license: pluginData.repository?.license?.key || 'MIT',
                                license_url: pluginData.repository?.license?.url || null,
                                readme_url: pluginData.repository?.readme_url || null,
                                repository_url: pluginData.repository?.html_url || null,
                                dependencies: pluginData.meta?.dependencies || {},
                                requirements: pluginData.meta?.requirements || []
                            };
                            pluginsArray.push(plugin);
                        }
                        
                        // 按时间排序：有时间的在前，没有时间的在后
                    ...
                    console.log('演示模式：成功加载插件数据，共', this.plugins.length, '个插件');
                    await new Promise(resolve => setTimeout(resolve, 300));
                } catch (error) {
                    console.error('演示模式：加载在线插件失败', error);
                    this.loading = false;
                    this.showNotificationMsg('演示模式：加载在线插件列表失败', 'error');
                }
                return;
            }
        ...
        async loadRepositories(currentRepoUrl = null) {
            if (this.isDemoMode) {
                try {
                    // 演示模式：直接使用真实的仓库URL
                    console.log('演示模式：加载仓库列表');
                    
                    // 添加官方仓库，编号为0
                    this.repositories = [{
                        name: '官方仓库',
                        url: 'https://api.mcdreforged.com/catalogue/everything_slim.json',
                        repoId: 0  // 官方仓库编号为0
                    }];
                    ...
                    console.log('演示模式：仓库列表加载完成', this.repositories);
                    await new Promise(resolve => setTimeout(resolve, 200));
                } catch (error) {
                    console.error('演示模式：加载仓库列表失败', error);
                    this.showNotificationMsg('演示模式：加载仓库列表失败', 'error');
                }
                return;
            }
            ...
            try {
                ...
                this.repositories = [{
                    name: '官方仓库',
                    url: data.mcdr_plugins_url || 'https://api.mcdreforged.com/catalogue/everything_slim.json',
                    repoId: 0  // 官方仓库编号为0
                }];
        ...
        filterPlugins() {
            ...
            if (!this.plugins || !Array.isArray(this.plugins)) return [];
        ...
        sortPlugins(pluginsArray) {
            if (!pluginsArray || !Array.isArray(pluginsArray) || pluginsArray.length === 0) return [];
        ...
        getTotalPages() {
            const filtered = this.filterPlugins();
            if (!Array.isArray(filtered)) return 1;
        ...
        async showPluginVersions(pluginId) {
            if (this.isDemoMode) {
                ...
                try {
                    ...
                    if (data && data.plugins && data.plugins[pluginId] && data.plugins[pluginId].release?.releases) {
                        // 从真实数据中获取版本信息
                        const pluginData = data.plugins[pluginId];
                        this.versions = pluginData.release.releases.map(release => ({
                            version: release.tag_name?.replace(/^v/, '') || release.name || '1.0.0',
                            release_date: release.created_at || new Date().toISOString(),
                            download_count: release.asset?.download_count || 0,
                        ...
                        this.versions.forEach(version => {
                            version.installed = version.version === this.installedVersion;
                        });
                        
                        console.log('演示模式：从真实数据加载版本信息完成', this.versions);
                    } else {
                        // 如果无法获取真实数据，使用模拟数据
                        const baseVersion = plugin.latest_version || plugin.version || '1.0.0';
                        ...
                        console.log('演示模式：使用模拟版本数据', this.versions);
                    }
        ...
        async switchPluginVersion(version) {
            if (this.isDemoMode) {
                this.showDemoModeNotification('版本切换');
                // 关闭版本选择模态框
        ...
        async installPluginDirectly(pluginId) {
            if (this.isDemoMode) {
                this.showDemoModeNotification('安装');
                return;
            }
        ...
        showDemoModeNotification(operation) {
            this.showNotificationMsg(`演示模式不支持${operation}操作`, 'info');
        },
